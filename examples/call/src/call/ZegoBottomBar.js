...
export default function ZegoBottomBar(props) {
    ...
    const getButtonByButtonIndex = (buttonIndex) => {
        switch (buttonIndex) {
            case 0:
                return <ZegoLeaveButton key={buttonIndex} onLeaveConfirmation={onHangUpConfirmation} onPressed={onHangUp} />
            case 1:
                return <ZegoToggleCameraButton key={buttonIndex} isOn={turnOnCameraWhenJoining} />;
            case 2:
                return <ZegoToggleMicrophoneButton key={buttonIndex} isOn={turnOnMicrophoneWhenJoining} />;
            case 3:
                return <ZegoSwitchCameraButton key={buttonIndex} />
            case 4:
                return <ZegoSwitchAudioOutputButton key={buttonIndex} useSpeaker={useSpeakerWhenJoining} />
        }
    }
